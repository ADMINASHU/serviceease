function sortAndMergeColumnsToIndentTable() {
    ...
    for (let i = 0; i < group.length; i++) {
      ...
      if (!exclusionSet.has(logCValue)) {
        for (let col = 24; col <= 30; col++) {
          row[col] = ""; // or use a valid value from the dropdown list
        }
      }
      if (i > 0) {
  ...
  const entryRangeFormat = entrySheet.getRange(1, 1, entrySheet.getLastRow(), 53);
  const indentRangeFormat = indentSheet.getRange(1, 1, sortedValues.length, 53);
